...
func _on_Timer_timeout():
	if $Node2D/plate/PathFollow2D.unit_offset < 0.99:
		$Node2D/plate/PathFollow2D.unit_offset += 0.005*PlVar.uldelta
		$Timer.start(0.001)
	else:
		if step == 0:
			$Timer2.start(1)
...
func _on_Timer2_timeout():
	if step == 2:
		if tunsound == 0:
			...
			$Sounds.play()
			tunsound = 1
		$Node2D/vaisseau/PathFollow2D/ship.global_position.y -= 1.5*PlVar.uldelta
